...
namespace MWPhysics
{
    ...
    // Calculate sliding direction based on slope normal
    osg::Vec3f MovementSolver::calculateSlideDirection(const osg::Vec3f& normal) {
        osg::Vec3f slideDirection;
        slideDirection._v[0] = normal.x();
        slideDirection._v[1] = normal.y();
        slideDirection._v[2] = 0.0f;
        slideDirection.normalize();
        return slideDirection;

    }

    // Helper to determine if slope is surfable
    bool MovementSolver::isSurfableSlope(const osg::Vec3f& normal) {
        static const float sMinSurfCos = std::cos(osg::DegreesToRadians(MWPhysics::sSurfThreshold));
        static const float sMaxSlopeCos = std::cos(osg::DegreesToRadians(MWPhysics::sMaxSlope));
        float cosAngle = normal.z();
        bool result = (cosAngle < sMaxSlopeCos && cosAngle > sMinSurfCos);
        std::cout << "isSurfableSlope - normal.z(): " << cosAngle << ", minCos: " << sMinSurfCos << ", maxCos: " << sMaxSlopeCos << ", result: " << result << std::endl;
        return result;
    }

    // after the surf changes so far i stick to VERY steep slopes without falling off but don't slide at all.
    ...
    {
        std::cout << "top of traceDown" << std::endl;
        ...
        if (tracer.mFraction >= 1.0f) {
            ...
            actor->setSliding(false);
            return position;
        }
    
        actor->setOnGround(true);
    
        // Check if we found a valid spawn point with an infinitely thin ray
        ...
        osg::Vec3f groundPosition;
        if (resultCallback1.hasHit()) {
            groundPosition = Misc::Convert::toOsg(resultCallback1.m_hitPointWorld);
            osg::Vec3f groundNormal = Misc::Convert::toOsg(resultCallback1.m_hitNormalWorld);
            
            bool walkable = isWalkableSlope(groundNormal);
            bool surfable = isSurfableSlope(groundNormal);
            
            std::cout << "pants were poopable." << std::endl;
            
            // Enable sliding on surfable slopes
            if (surfable && !walkable) {
                std::cout << "slope was surfable." << std::endl;
                actor->setOnGround(false);
                actor->setSliding(true);
                osg::Vec3f slideDirection =  calculateSlideDirection(groundNormal);
                osg::Vec3f slidingMovement = slideDirection * MWPhysics::sSlideSpeed; //* deltaTime;  // `slideSpeed` defines speed on surf slopes
                std::cout << "Sliding Movement: " << slidingMovement.x() << ", " << slidingMovement.y() << ", " << slidingMovement.z() << std::endl;
                return position + slidingMovement + osg::Vec3f(0, 0, sGroundOffset);
            } else {
                std::cout << "slope was NOT surfable." << std::endl;
                actor->setOnGround(true);
                actor->setSliding(false);
                actor->setOnSlope(!walkable);
                return groundPosition + osg::Vec3f(0.f, 0.f, sGroundOffset);
            }
        }
    ...
    {
        // std::cout << "top of MovementSolver::move." << std::endl;
        // Call traceDown to handle ground detection and potential sliding
        // osg::Vec3f newPosition = traceDown(actor.ptr, actor.mPosition, physicActor, collisionWorld, /*maxHeight*/ 100.0f);
        // actor.mPosition = newPosition;  // Update actor position based on traceDown results
        ...
        {
            // std::cout << "SURF ADDITION RAN" << std::endl;
            ...
            //     inertia = velocity;
            // SURF ADDITION begin
            if (physicActor->getSliding()) {
                // std::cout << "if (physicActor->getSliding())" << std::endl;
                osg::Vec3f slideDirection = calculateSlideDirection(physicActor->getSlopeNormal());
                velocity = slideDirection * MWPhysics::sSlideSpeed;  // Use slideSpeed for sliding velocity
            } 
            else if (physicActor->getOnGround()) {
                physicActor->setOnGround(false);
                
                // std::cout << "else if (physicActor->getOnGround())" << std::endl;
                inertia = velocity;
            }
            else if (!physicActor->getOnGround() || physicActor->getOnSlope()) {
                // std::cout << "else if (!physicActor->getOnGround() || physicActor->getOnSlope())" << std::endl;
        ...
        */
        ...
        // std::cout << "Inertia: ("
        //     << std::fixed << std::setprecision(2) // Set fixed-point notation and precision
        //     << inertia.x() << ", "
        //     << inertia.y() << ", "
        //     << inertia.z() << ") "
        //     << std::endl;
        physicActor->setOnGround(false);
        physicActor->setOnSlope(false);
        //inertia.x() *= 5.0f; 
        inertia.z() -= time * Constants::GravityConst * Constants::UnitsPerMeter; //adjust these values so inertia chages when you are sliding on a ramp to simulate friction (but very low amounts.)
        physicActor->setInertialForce(inertia);
        // physicActor->setInertialForce(osg::Vec3f(0.f, 0.f, 0.f));


        /*osg::Vec3f vel = physicActor->getVelocity();
        std::cout << "Velocity: X=" << vel.x() << ", Y=" << vel.y() << ", Z=" << vel.z() << std::endl;*/

        // SURFMOVEMENTS ADDITION begin
        // if (tracer.mFraction < 1.0f && !actor.mFlying)  // If we hit something and aren't flying
        // {
        //     btVector3 normal = Misc::Convert::toBullet(tracer.mPlaneNormal);
            
        //     // Apply surf mechanics if on appropriate surface
        //     if (SurfMovements::isSurfableSurface(normal))
        //     {
        //         SurfMovements::handleSurfMovement(actor, normal, time);
                
        //         // Apply surfing effects if this is the player
        //         if (actor.mActorRaw->getPtr() == MWBase::Environment::get().getWorld()->getPlayerPtr())
        //             SurfMovements::applySurfingEffects(actor.mActorRaw->getPtr());
                
        //         // Update actor's surfing state
        //         SurfMovements::updateSurfingState(actor.mActorRaw, normal);
        //     }
        // }
