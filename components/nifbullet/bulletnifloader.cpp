...
constexpr int MAX_TRIANGLES_PER_SHAPE = 5000;

namespace
{
    void fillTriangleList(
        std::vector<TempTriangle>& out,
        const Nif::NiGeometry* geometry,
        const osg::Matrixf& transform)
    {
        if (!geometry || geometry->data.empty())
            return;

        if (geometry->recType == Nif::RC_NiTriShape
            || geometry->recType == Nif::RC_BSLODTriShape)
        {
            const auto& d =
                static_cast<const Nif::NiTriShapeData&>(geometry->data.get());
            ...
            {
                out.push_back({
                    Misc::Convert::toBullet(vertices[tris[i]] * transform),
                    Misc::Convert::toBullet(vertices[tris[i + 1]] * transform),
                    Misc::Convert::toBullet(vertices[tris[i + 2]] * transform)
                    });
            }
        }
        ...
        {
            const auto& d =
                static_cast<const Nif::NiTriStripsData&>(geometry->data.get());

            const auto& vertices = d.vertices;

            for (const auto& strip : d.strips)
            {
                for (size_t i = 2; i < strip.size(); ++i)
                {
                    unsigned short a = strip[i - 2];
                    unsigned short b = strip[i - 1];
                    unsigned short c = strip[i];
...
namespace
{
    // Iterative version to avoid recursion overhead/deep stacks for deep node hierarchies.
    osg::Matrixf getWorldTransform(const Nif::Node* node)
    {
        ...
        const Nif::Node* cur = node;
        // To avoid extra recursion, store transforms in a small vector then multiply in reverse.
        std::vector<const Nif::Node*> stack;
        while (cur)
        {
            stack.push_back(cur);
            cur = cur->parent;
        ...
        {
            result = result * (*it)->trafo.toMatrix();
        }
        return result;
    }
    ...
    // Keep signature and behavior exactly the same, but avoid repeated copies and
    // do small preallocations / use emplace_back for performance.
    void fillTriangleMesh(btTriangleMesh& mesh, const Nif::NiTriShapeData& data, const osg::Matrixf& transform)
    {
        ...
        // preallocate (helps performance)
        mesh.preallocateVertices(static_cast<int>(vertices.size()));
        mesh.preallocateIndices(static_cast<int>(triangles.size()));

        // iterate safely; ensure we don't access out of range if triangles.size() isn't multiple of 3
    ...
    void fillTriangleMesh(btTriangleMesh& mesh, const Nif::NiTriStripsData& data, const osg::Matrixf& transform)
    {
        ...
        // estimate triangles to reserve memory once
        ...
        // reserve index count (3 indices per triangle)
        mesh.preallocateIndices(static_cast<int>(numTriangles * 3));

        // Triangulate strips into local flat list of indices (avoid repeated insert cost)
        std::vector<unsigned short> allTriangles;
        allTriangles.reserve(numTriangles * 3);

        for (const auto& strip : strips)
        {
            const size_t s = strip.size();
            if (s < 3)
                continue;

            // Use sliding window to generate triangles
            ...
            for (size_t i = 2; i < s; ++i)
            {
                ...
                if (i % 2 == 0)
                {
                    allTriangles.push_back(a);
                    allTriangles.push_back(b);
                    allTriangles.push_back(c);
                }
                else
                {
                    allTriangles.push_back(a);
        ...
        // Add triangles to mesh (safe iteration)
        const size_t m = allTriangles.size();
        for (size_t i = 0; i + 2 < m; i += 3)
        {
            mesh.addTriangle(
                Misc::Convert::toBullet(vertices[allTriangles[i + 0]] * transform),
                Misc::Convert::toBullet(vertices[allTriangles[i + 1]] * transform),
                Misc::Convert::toBullet(vertices[allTriangles[i + 2]] * transform)
            );
        }
    }

    // Keep same signature and default behaviour
    void fillTriangleMesh(btTriangleMesh& mesh, const Nif::NiGeometry* geometry, const osg::Matrixf& transform = osg::Matrixf())
    {
        ...
        if (geometry->recType == Nif::RC_NiTriShape || geometry->recType == Nif::RC_BSLODTriShape)
        {
            ...
            if (geometry->data->recType != Nif::RC_NiTriShapeData)
                return;
            const Nif::NiTriShapeData& d = static_cast<const Nif::NiTriShapeData&>(geometry->data.get());
            fillTriangleMesh(mesh, d, transform);
        }
        else if (geometry->recType == Nif::RC_NiTriStrips)
        {
            if (geometry->data->recType != Nif::RC_NiTriStripsData)
                return;
            const Nif::NiTriStripsData& d = static_cast<const Nif::NiTriStripsData&>(geometry->data.get());
            fillTriangleMesh(mesh, d, transform);
        }
    }
...
namespace NifBullet
{
    ...
    osg::ref_ptr<Resource::BulletShape> BulletNifLoader::load(const Nif::File& nif)
    {
        ...
        if (mCompoundShape)
        {
            // On n'ajoute mStaticMesh que s'il contient réellement des données
            if (mStaticMesh && mStaticMesh->getNumTriangles() > 0)
            {
                btTransform trans;
                trans.setIdentity();
            ...
            // Si mStaticMesh était vide ou nul, on l'ignore simplement pour éviter le crash
            mShape->mCollisionShape = mCompoundShape.release();
        }
        else if (!mStaticTriangles.empty())
        {
            if (mStaticTriangles.size() <= MAX_TRIANGLES_PER_SHAPE)
            {
                auto mesh = new btTriangleMesh(false);
                for (const auto& t : mStaticTriangles)
                    mesh->addTriangle(t.v0, t.v1, t.v2);

                mShape->mCollisionShape =
                    new Resource::TriangleMeshShape(mesh, true);
            }
            else
            {
                ...
                size_t index = 0;
                while (index < mStaticTriangles.size())
                {
                    auto subMesh = new btTriangleMesh(false);

                    int count = 0;
                    while (index < mStaticTriangles.size()
                        && count < MAX_TRIANGLES_PER_SHAPE)
                    {
                        const auto& t = mStaticTriangles[index++];
                        subMesh->addTriangle(t.v0, t.v1, t.v2);
                        count++;
                    }

                    auto subShape =
                        new Resource::TriangleMeshShape(subMesh, true);

                    btTransform tr;
                    tr.setIdentity();
                    compound->addChildShape(tr, subShape);
                }

                mShape->mCollisionShape = compound;
            }
        ...
        bool isAnimated, bool avoid)
    {
        ...
        else
        {
            ...
            fillTriangleList(mStaticTriangles, niGeometry, transform);
        }
    }
