...
namespace MWPhysics
{
    ...
    bool isWalkableSlopeSurf(const osg::Vec3f& normal)
    {
        float zComponent = normal.z();
        bool walkable = zComponent >= RAMP_ANGLE; // make this value configurable in game via lua commands
    ...
    void MovementSolver::move(ActorFrameData& actor, float time, const btCollisionWorld* collisionWorld, WorldFrameData& worldData)
    {
        if (SURF_PHYSICS_ENABLED)
        {
            moveSurf(actor, time, collisionWorld, worldData);
        }
        else
        {
            moveVanilla(actor, time, collisionWorld, worldData);
        }
    }

    void MovementSolver::moveVanilla(ActorFrameData& actor, float time, const btCollisionWorld* collisionWorld,
        ...
        WorldFrameData& worldData)
    {
        ...
        for (int iterations = 0; iterations < sMaxIterations && remainingTime > 0.0001f; ++iterations)
        {
            ...
            if (isWalkableSlopeSurf(tracer.mPlaneNormal) && !actor.mFlying && newPosition.z() >= swimlevel)
                seenGround = true;
        ...
        if (!actor.mFlying && !isSwimming && forceGroundTest || (velocity.z() <= 0.f && newPosition.z() >= swimlevel))
        {
            ...
            if (tracer.mFraction < 1.0f)
            {
                if (!isActor(tracer.mHitObject))
                {
                    ...
                    isOnSlopeFinal = !isWalkableSlopeSurf(tracer.mPlaneNormal);
        ...
        physicActor->setOnGround(isOnGroundFinal); //figure out how to make this false when your on a surfable ramp.
